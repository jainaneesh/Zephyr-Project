...
user_iram_end = (0x3fcd7e00 + 0x6f0000);
user_dram_seg_org = 0x3fc88000;
user_iram_seg_org = 0x40370000 + 0x4000;
user_dram_end = (0x3fcd7e00 - 0x0 + 0x6f0000 - 0x1a00) - 0x6f0000;
user_idram_size = user_dram_end - 0x3fc88000;
sram0_iram_size = 0x8000 - 0x4000;
user_iram_seg_len = user_idram_size + sram0_iram_size;
user_dram_seg_len = user_idram_size;
MEMORY
{
  FLASH (R): org = 0x0, len = 8388608 - 0x100
  iram0_0_seg(RX): org = user_iram_seg_org, len = user_iram_seg_len - 0
  dram0_0_seg(RW): org = user_dram_seg_org, len = user_dram_seg_len - 0
  irom0_0_seg(RX): org = 0x42000000, len = 8388608
  drom0_0_seg(R): org = 0x3c000000, len = 8388608
 ...
 *(.rela.*)
  ...
  .dram0.dummy (NOLOAD):
  {
    . = ORIGIN(dram0_0_seg) + MAX(_iram_end, (0x40370000 + 0x8000)) - (0x40370000 + 0x8000);
    . = ALIGN(4) + 16;
  } > dram0_0_seg
  .dram0.data :
  {
 ...
 log_link_area : ALIGN_WITH_INPUT { _log_link_list_start = .; KEEP(*(SORT_BY_NAME(._log_link.static.*))); _log_link_list_end = .; } > dram0_0_seg AT > FLASH
  ...
  _image_ram_start = _iram_start - 0x6f0000;
